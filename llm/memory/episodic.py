...
class EpisodicMemoryProvider:
    ...
    def __init__(self, bot: Any, top_k: int = 3, max_chars: int = 1500, max_cache_size: int = 1000, cache_ttl: float = 300.0) -> None:
        ...
        # Protect against thundering herd (multiple concurrent identical queries missing cache)
    ...
    async def get(self, message: discord.Message) -> Optional[str]:
        ...
        if cache_key in self._pending_queries:
            ...
            if entry is not None and entry[1] > time.monotonic():
                return entry[0]
            # If it's still missing, we could retry or just proceed.
            # Usually the first task would populate the cache.
