...
try:
    ...
    asyncio.create_task(func.func.report_error(e, "Gemini API client initialization"))
    raise
...
def _download_and_process_pdf(pdf_url_or_path: str) -> pathlib.Path:
    ...
    try:
        ...
        asyncio.create_task(func.func.report_error(e, "PDF processing"))
    ...
    try:
        ...
        async def upload_single_pdf(pdf_path, index):
            ...
            try:
                ...
                await func.func.report_error(e, f"PDF upload failed for index {index+1}")
                raise
        ...
        await func.func.report_error(e, "PDF upload failed")
        raise
    finally:
        # 清理暫存檔案（如果是下載的）
        import os
        for temp_path in temp_files:
            try:
                ...
                asyncio.create_task(func.func.report_error(cleanup_error, "Failed to clean up temporary PDF file"))
    ...
    try:
        ...
        async def upload_single_file(temp_path, index):
            ...
            try:
                ...
                await func.func.report_error(e, f"{media_type} upload failed for index {index+1}")
                raise
        ...
        await func.func.report_error(e, f"{media_type} upload failed")
        raise
    finally:
        # 清理臨時檔案
        import os
        for temp_path in temp_files:
            try:
                ...
                asyncio.create_task(func.func.report_error(cleanup_error, "Failed to clean up temporary media file"))
...
async def _build_conversation_contents(inst, dialogue_history=None, image_input=None, audio_input=None, video_input=None, pdf_input=None):
    ...
    if pdf_input:
        ...
        try:
            ...
            await func.func.report_error(e, "PDF processing in conversation build")
            # 降級：修改文字提示
        ...
        try:
            ...
            await func.func.report_error(e, "video processing in conversation build")
            # 降級：修改文字提示
        ...
        try:
            ...
            await func.func.report_error(e, "audio processing in conversation build")
        ...
        try:
            ...
            await func.func.report_error(e, "image processing in conversation build")
            # 降級：使用 inlineData 格式
    ...
    def hash_media_content(media_input, media_type):
        ...
        for item in inputs:
            ...
            try:
                ...
                asyncio.create_task(func.func.report_error(e, f"hashing media content for {media_type}"))
...
async def generate_response(inst: str,
    ...
    if use_cache and cache_mgr:
        ...
        if cache is None:
            try:
                ...
                await func.func.report_error(e, "dynamic cache creation")
    ...
    if use_cache and cache:
        try:
            ...
            await func.func.report_error(e, "cached content generation")
    ...
    try:
        ...
        await func.func.report_error(e, "processing Gemini response")
