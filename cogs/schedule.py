...
class ScheduleManager(commands.Cog):
    ...
    async def query_schedule_command(self, interaction: discord.Interaction, query_type: app_commands.Choice[str], time: str = None, day: str = None):
        await interaction.response.defer(thinking=True)
        try:
            result = await self.query_schedule(interaction.user.id, query_type.value, time, day)
            await interaction.followup.send(result)
        except Exception as e:
            await interaction.followup.send(f"查詢行程表時發生錯誤：{str(e)}")

    @commands.command(name="query_schedule", description="查詢行程表")
    async def query_schedule_general(self, ctx: commands.Context, query_type: str, time: str = None, day: str = None):
        try:
            result = await self.query_schedule(ctx.author.id, query_type, time, day)
            await ctx.send(result)
        except Exception as e:
            await ctx.send(f"查詢行程表時發生錯誤：{str(e)}")


    async def query_schedule(self, user_id: int, query_type: str, time: str = None, day: str = None):
